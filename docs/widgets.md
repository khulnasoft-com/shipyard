...
Shipyard has support for displaying dynamic content in the form of widgets. There are several built-in widgets available out-of-the-box as well as support for custom widgets to display stats from almost any service with an API.
...
- type: image
  options:
    imagePath: https://i.ibb.co/yhbt6CY/shipyard.png
```
...
**Field** | **Type** | **Required** | **Description**
--- | --- | --- | ---
**`ipAddress`** | `string` |  _Optional_ | The IP to check. This can also be a domain/ host name or even an email address. If left blank, Shipyard will use your current public IP address.
...
Display your coding summary. [Code::Stats](https://codestats.net/) is a free and open source app that aggregates statistics about your programming activity. Shipyard supports both the public instance, as well as self-hosted versions.
...
**`repos`** | `array` |  _Optional_ | If you'd like to also display stats for some GitHub repositories, then add an array or repo names here. Specified as `[username]/[repo-name]`, e.g. `khulnasoft/shipyard`
...
This widget allows searching multiple search engines from shipyard.
```yaml
  - type: 'custom-search'
    options:
...
Displays info about the server which Shipyard is hosted on. Includes user + host, operating system, uptime and basic memory & load data.
...
Note that this widget is not available if you are running Shipyard in a container or VM. Instead you can use the [System Monitoring](#system-resource-monitoring) widgets to display stats from the host system instead.
...
**`user_name`** | `string` |  Required | A Proxmox API Username, for example `root@pam` or `shipyard@pve`.
...
- **500 Error in production mode**: Try adding the certificate authority (CA) certificate of your Proxmox host to Node.js. 
  - Download the Proxmox CA certificate to your Shipyard host.
...
### Glances
The easiest method for displaying system info and resource usage in Shipyard is with [Glances](https://nicolargo.github.io/glances/).
...
Many websites and apps provide their own embeddable widgets. These can be used with Shipyard using the Embed widget, which lets you dynamically embed and HTML, CSS or JavaScript contents.
...
- type: embed
  options:
    html: '<img src="https://shipyard.lan/item-icons/my-image.png" />'
```
...
Alternatively, and more securely, you can set the auth headers on your service to accept requests from Shipyard. For example:
...
Like elsewhere in Shipyard, all colours can be easily modified with CSS variables.
...
The most common issue is a CORS error. This is a browser security mechanism which prevents the client-side app (Shipyard) from from accessing resources on a remote origin, without that server's explicit permission (e.g. with headers like Access-Control-Allow-Origin). See the MDN Docs for more info: [Cross-Origin Resource Sharing](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS).
...
Add the `Access-Control-Allow-Origin` header, with the value of either `*` to allow requests from anywhere, or more securely, the host of where Shipyard is served from. For example:

```text
Access-Control-Allow-Origin: https://url-of-shipyard.local
```
...
You can route requests through Shipyard's built-in CORS proxy. Instructions and more details can be found [here](#proxying-requests). If you don't have control over the target origin, and you are running Shipyard either through Docker, with the Node server or on Netlify, then this solution will work for you.
